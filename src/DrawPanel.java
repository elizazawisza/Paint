...
public class DrawPanel extends JPanel implements ActionListener {
    ...
    MyColorItem objcolor;

    /**
     * Konstruktor klasy
     * @param f  okno główne
     * @param panel  panel, na którym wyświetlane są informacje dotyczące aktualnego położenia figury przy ruszaniu nią
    ...
    DrawPanel(Frame f, InformationPanel panel, ArrayList<Shape> shapes, ArrayList<Color> colors){
        ...
        MyDrawingListener draw_listener  = new MyDrawingListener();
        addMouseListener(movea);
        addMouseMotionListener(movea);
        addMouseListener(draw_listener);
        addMouseMotionListener(draw_listener);
    ...
    /**
     * Klasa, która służy o rysowania figur geometrycznych. Wielokąt jest rysowany poprzez łączenie punktów, w których
     * użytkownik kliknął myszką. Ostatni punkt, po którym następuje narysowanie wielokąta jest oznaczany poprzez
     * podówójne kliknięcie myszką w jednym miejscu.
     * Rysowanie koła i prostokąta następuje po naciśnieciu myszki, przeciągnięciu kursora myszki po ekranie, a następnie
    ...
    class MyDrawingListener extends MouseAdapter {
        /**
         * Metoda dzięki którejnastępuje rysowanie wielokąta oraz zmiana koloru aktualnie klikniętej figury w zależności
         * od tego która opcja modify zostanie wybrana oraz króry przycisk myszy zostanie klikniety.
         * @param me  akcja wywołana przez użytkownika
         */
        public void mouseClicked(MouseEvent me){
            ...
            if(shapetype == "Polygon"){
                ...
                int[] p_x = new int[points_x.size()];
                int[] p_y = new int[points_x.size()];
                for(int i=0; i<p_x.length; i++){
                    p_x[i] = points_x.get(i).intValue();
                    p_y[i] = points_y.get(i).intValue();
                }
                if(polygonready){
                    shape = new Polygon(p_x, p_y, p_x.length);
                }
                if (shape != null) {
                    shapes.add(shape);
                    setcol();
        ...
        /**
         * Metoda pobierająca współrzędne x i y początkowego punktu naciśnięcia myszki podczas rysowania figury
    ...
    /**
     * Klasa odpowiedzialna za modyfikowanie figury.  Zmiana położnenia figury następuje po kliknięciu na nią,
     * a następnie przejechaniu figurą w miejsce, w które chcemy przenieść figurę. Zmiana położenia figury możliwa jest
     * tylko po wcześniejszym wybraniu opcji 'moving' w menu modify.
     */
    ...
    /**
     *  Klasa odpowiedzialna za modyfikowanie figury. Zmiana rozmiaru następuje poprzez scrollowanie. Zmiana rozmiaru
     *  jest możliwa tylko po wcześniejszym wybraniu opcji 'resizing' w menu modify.
     */
                ...
                for(int i=0; i<shapes.size(); i++){
                    ...
                    if (e.getScrollType() == MouseWheelEvent.WHEEL_UNIT_SCROLL) {
                        if(s.contains(x, y) && s instanceof Rectshape){
                            ...
                            double p_x = s.getBounds().getX()+s.getBounds().getWidth()/2;
                            double p_y = s.getBounds().getY()+s.getBounds().getHeight()/2;
                            at.scale(amount,amount);
                            Shape tmpshape = at.createTransformedShape(s);
                            at = new AffineTransform();
                            at.translate(
                                    p_x - tmpshape.getBounds().getX()-tmpshape.getBounds().getWidth()/2,
                                    p_y - tmpshape.getBounds().getY()-tmpshape.getBounds().getHeight()/2
                            );
